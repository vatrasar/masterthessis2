...
def check_distance_between_uav(uav_list:typing.List[Uav],save_distance):
    if len(uav_list)<2:
        return True
    if get_distance_on_tier1(uav_list[0].position,uav_list[1].position)<save_distance:
        return False
    else:
        return True
...
def chekc_if_uav_goes_to_trash(event_owner, potential_crash_point, target_postion,save_distance):
    distance_to_trash=get_distance_on_tier1(event_owner.position,potential_crash_point)
    distance_to_target=get_distance_on_tier1(event_owner.position,target_postion)
