...
class Move_along(Event):
    ...
    def handle_event(self, event_list,settings:Settings,rand:Random,iteration_function):
        ...
        if decide_whether_uav_attack(settings.mode,settings.prob_of_attack,rand) and check_if_in_safe_distance(self.event_owner,self.state.hands_list,self.safe_margin) and False:
            pass
        else: #no attack
