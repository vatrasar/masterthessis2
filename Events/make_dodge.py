...
class Make_dodge(Event):
    def __init__(self, time_of_event, event_owner,tk_master,target_postion,next_status,old_target,new_target,game_state,safe_margin):
    ...
    def handle_event(self, event_list:Event_list,settings:Settings,rand:Random,iteration_function):
        ...
        if self.event_owner.status==UavStatus.DODGE:
            ...
            event=Make_dodge(event_time,self.event_owner,self.tk_master,new_target,UavStatus.BACK_FROM_DODGE,self.old_target,None,self.game_state,settings.safe_margin)
            ...
            event=Make_dodge(event_time,self.event_owner,self.tk_master,self.new_target,UavStatus.DODGE,self.event_owner.position,self.old_target,self.game_state,settings.safe_margin)
