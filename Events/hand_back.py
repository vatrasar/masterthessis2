...
from Events.Game.move.GameObjects.tools.enum.enumStatus import HandStatus, UavStatus
...
def plan_hand_back_event(event_list, settings,hand,game_state,time_of_event,tk_master):
        ...
        if check_if_point_is_reached(settings.velocity_hand, settings.minimal_travel_time, hand.position,
            ...
            new_event = Hand_back(time_of_event, hand, tk_master, HandStatus.BACK, tier0_pos,
                                  game_state)
            event_list.append_event(new_event, HandStatus.TIER_0)
        else:
